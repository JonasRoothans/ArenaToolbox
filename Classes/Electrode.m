classdef Electrode < handle & matlab.mixin.Copyable & ArenaActorRendering
    ...
    methods
        ...
        function VTAObject = makeVTA(obj,vtaname)
            ...
            availableConversion = {'Medtronic33873False60c1 0 0 0a0 0 0 0.mat','Medtronic33873False60c0 1 0 0a0 0 0 0.mat', ...
                'Medtronic33873False60c0 0 1 0a0 0 0 0.mat','Medtronic33873False60c0 0 0 1a0 0 0 0.mat'}; % these are all VTAs which we can convert now (needs update when more time)
            
            NameCell(1:numel(availableConversion)) = {vtaname};
            
          
            
            for ii = 1:numel (availableConversion)
                
                available(ii) = isequal(NameCell{ii},availableConversion{ii});
             ...
             VTA_raw = load(fullfile(arena.Settings.VTApool,vtaname));
                ...
                if isfolder(arena.Settings.VTApool)
                    ...
                    if available
                        ...
                        warning ('Your VTA is not in the pool,transformation from 3389 to 3387 will be used')
                        
                        VTA_vd = Electrode.Medtronic9to7(vtaname);
