            ...
            for i=1:Actor.PredictInformation.configStructure.numberOfContacts
                ...
                try
                    if  not(isempty(Actor.PredictInformation.Results{1, 1}.unilateral.left))
                        ...
                        side='left';
                        try
                            creationOfObjects(thisScene,Actor,greaterThanThreshold,bar,barText,...
                            position,positionAmplitude,side,i,Actor.PredictInformation.confidenceLevel.left.average);
                        catch
                            creationOfObjects(thisScene,Actor,greaterThanThreshold,bar,barText,...
                            position,positionAmplitude,side,i,Actor.PredictInformation.confidenceLevel);
                        end
                    end
               catch
                end
                    if not(isempty(Actor.PredictInformation.Results{1, 1}.unilateral.right))
                        ...
                        creationOfObjects(thisScene,Actor,greaterThanThreshold,bar,barText,...
                            ...
                            creationOfObjects(thisScene,Actor,greaterThanThreshold,bar,barText,...
                            position,positionAmplitude,side,i,Actor.PredictInformation);
                        end
                    end
            end
            
            function creationOfObjects(thisScene,Actor,greaterThanThreshold,bar,barText,position,positionAmplitude,side,i,confidenceLevel)
                ...
                if not(isempty(valueOfStimulation))
                    ...
                    if isa(confidenceLevel,'PredictInformation')
