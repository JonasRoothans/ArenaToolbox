        ...
        function [Excel_output,Patient_Information,result]=VTA_Transformation(obj,thisprediction)
            ...
            if result==0
                ...
                for itherapyPlanStorage=1:numel(leadidcs)
                    ...
                    thisprediction.handles.TransformationLegacySpace{itherapyPlanStorage} =...            % this is the euqivalent of XLS.Tlead2MNI.value
                ...
                switch thisprediction.Heatmap.Name
                    ...
                    thisprediction.Heatmap.Data.loadnii('BER_MDST_UPDRS_corrdata_exch_palm_vox_vstat_R_Fz_yeo1000_dil_map.nii',1);
                    case 'heatmapBostonAlone'
                    thisprediction.Heatmap.Data=VoxelData;
                    thisprediction.Heatmap.Data.loadnii('cogdec_SE_vs_noSE_orig_fz_use_design_palm_vox_tstat_yeo1000_dil_precomputed_corr_R_Fz.nii',1);
                    otherwise
                ...
                for iLead =1: numel(leadidcs)
                    ...
                    for iStimplan = 1:numel(thisLead.stimPlan)
                        if thisprediction.saveLoadedVTA
                            lead.ExporttoVTApool(thisprediction,thisLead.stimPlan{iStimplan});
                        end
                    end
                    [Ivta,Rvta]=lead.getVTAInformation(thisLead.stimPlan{iStimplan});
                    thisVTA={Ivta,Rvta};
